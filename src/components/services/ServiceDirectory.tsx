...
import type { DirectoryCard, ServiceCategoryKey } from "@/data/advanced-services-content";

type DirectoryCategory = { key: ServiceCategoryKey; tabLabel: string; cardLabel: string };

function parseFilterParam(value: string | null, categoryKeys: ServiceCategoryKey[]): ServiceCategoryKey[] {
  if (!value) return [];
  return value
    .split(",")
    .map((key) => key.trim())
...
function ServiceDirectoryContent({
  ...
  const cardLabelByCategory = useMemo(
    () => Object.fromEntries(categories.map((category) => [category.key, category.cardLabel])) as Record<ServiceCategoryKey, string>,
    [categories],
  );
  ...
  const [activeFilters, setActiveFilters] = useState<Set<ServiceCategoryKey>>(
    () => new Set(parseFilterParam(searchParams.get("filter"), categoryKeys)),
  );
  ...
  }, [activeFilters, query, services, cardLabelByCategory]);
      ...
      <div className="container-brand">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <h2 className="mt-4 text-4xl md:text-6xl">{heading}</h2>
          </div>
...
export function ServiceDirectory(props: {
  eyebrow: string;
  heading: string;
  services: DirectoryCard[];
  categories: DirectoryCategory[];
